...
public class Options2 implements Parcelable {
    ...
    protected Options2(Parcel in) {
        OPTION_ID = in.readInt();
        OPTION_ANSWER = in.readInt();
        OPTION = in.readString();
    }

    public static final Creator<Options2> CREATOR = new Creator<Options2>() {
        @Override
        public Options2 createFromParcel(Parcel in) {
            return new Options2(in);
        }

        @Override
        public Options2[] newArray(int size) {
            return new Options2[size];
        }
    };
